    ...
    <div class="content">
        ...
        <div class="adm_table">
          <div class="add_admin">
            ...
            <form action="edit_art.php" method="POST">
              <p><input type="hidden"  name="edit_id" value="<?=$id?>"></p>
              <p><input type="text"  name="edit_title" value="<?=$title?>"></p>
              <textarea name="edit_text" id="" cols="70" rows="15"><?=$art_text?></textarea>
              <p> select category
              <select name="edit_selected_ctg" id="">
                <?php foreach($categories as $key => $category): ?>
                <option value="<?=$category['id']?>"><?=$category['ctg_name']?></option>
                <?php endforeach; ?>
              </select>
              </p>
              <p><input type="submit" value="add" class="add_btn" name="edit_article"></p>
           </form>
          </div>
        </div>
    </div>
